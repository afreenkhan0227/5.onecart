...
function Nav() {
  ...
  return (
    <div className='w-[100vw] h-[70px] bg-[#b3997f] z-10 fixed top-0 flex  items-center justify-between px-[30px] shadow-md shadow-black '>

        <div className='w-[20%] lg:w-[30%] flex items-center justify-start   gap-[10px] '>
            <img src={logo} alt="" className='w-[150px] mt-2 ml-6' />
            <h1 className='text-[25px] text-[black] font-sans '></h1>
        </div>
        <div className='w-[50%] lg:w-[40%] hidden md:flex'>
            <ul className='flex items-center justify-center gap-[19px] text-[black] '>
                <li className='text-[15px] hover:bg-[#b3997f] cursor-pointer bg-[#997d60] py-[10px] px-[20px] rounded-2xl' onClick={()=>navigate("/")}>HOME</li>
                <li className='text-[15px] hover:bg-[#b3997f] cursor-pointer bg-[#997d60] py-[10px] px-[20px] rounded-2xl' onClick={()=>navigate("/collection")}>COLLECTIONS</li>
                <li className='text-[15px] hover:bg-[#b3997f] cursor-pointer bg-[#997d60] py-[10px] px-[20px] rounded-2xl' onClick={()=>navigate("/about")}>ABOUT</li>
                <li className='text-[15px] hover:bg-[#b3997f] cursor-pointer bg-[#997d60] py-[10px] px-[20px] rounded-2xl' onClick={()=>navigate("/contact")}>CONTACT</li>
            </ul>
        </div>
       ...
       {showSearch && <div className='w-[100%]  h-[80px] bg-[#c8b9a9] absolute top-[100%] left-0 right-0 flex items-center justify-center '>
            <input type="text" className='lg:w-[50%] w-[80%] h-[60%] bg-[#b3997f] rounded-[30px] px-[50px] placeholder:text-black text-[black] text-[18px]' placeholder='Search Here' onChange={(e)=>{setSearch(e.target.value)}} value={search} />
        </div>}

       {showProfile && <div className='absolute w-[220px] h-[150px] bg-[#b3997f] top-[110%] right-[4%] border-[1px] border-[black] rounded-[10px] z-10'>
        <ul className='w-[100%] h-[100%] flex items-start justify-around flex-col text-[17px] py-[10px] text-[black]'>
            ...
            {userData && <li className='w-[100%] hover:bg-[#c8b9a9]  px-[15px] py-[10px] cursor-pointer' onClick={()=>{handleLogout();setShowProfile(false)}}>LogOut</li>}
            <li className='w-[100%] hover:bg-[#c8b9a9]  px-[15px] py-[10px] cursor-pointer'onClick={()=>{navigate("/order");setShowProfile(false)}} >Orders</li>
            <li className='w-[100%] hover:bg-[#c8b9a9]  px-[15px] py-[10px] cursor-pointer'onClick={()=>{navigate("/about");setShowProfile(false)}} >About</li>
        </ul>

        </div>}
        <div className='w-[100vw] h-[90px] flex items-center justify-between px-[20px] text-[12px]
         fixed bottom-0 left-0 bg-[#b3997f]   md:hidden'>
            <button className='text-[black] flex items-center justify-center flex-col gap-[2px]' onClick={()=>navigate("/")}><IoMdHome className='w-[28px] h-[28px] text-[black] md:hidden'/> Home</button>
             <button className='text-[black] flex items-center justify-center flex-col gap-[2px]' onClick={()=>navigate("collection")}><HiOutlineCollection className='w-[28px] h-[28px] text-[black] md:hidden'/> Collections</button>
              <button className='text-[black] flex items-center justify-center flex-col gap-[2px] ' onClick={()=>navigate("/contact")}><MdContacts className='w-[28px] h-[28px] text-[black] md:hidden'/>Contact</button>
               ...
               <p className='absolute w-[18px] h-[18px] flex items-center justify-center bg-[black] px-[5px] py-[2px] text-white font-semibold  rounded-full text-[9px] top-[8px] right-[18px]'>{getCartCount()}</p>
